...
export default function ButtonWhatsApp({ className }: ButtonWhatsAppProps) {
  ...
  return (
    <Button
      ...
      className={`flex items-center gap-2 bg-secondary hover:bg-secondary/90 text-foreground w-full rounded-none py-4 md:py-8 text-lg font-semibold hover:cursor-pointer ${className}`}
      onClick={handleClick}
    >
      <WhatsAppIcon />
      FALAR PELO WHATSAPP
